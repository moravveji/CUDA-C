...
__global__ void sumMatrixOnGPU_Grid2D_Block_1D(float * ma, float * mb, float * mc, const size_t nx, const size_t ny) {
  size_t ix = threadIdx.x + blockIdx.x * blockDim.x;
  size_t iy = blockIdx.y;
  size_t id = iy * nx + ix;
...
int main(int argc, char ** argv) {
  ...
  sumMatrixOnGPU_Grid2D_Block_1D<<< grid, block >>> (d_ma, d_mb, d_mc, nx, ny);
