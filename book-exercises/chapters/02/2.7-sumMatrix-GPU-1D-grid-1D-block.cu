...
__global__ void sumMatrixOnGPU_Grid1D_Block_1D(float * ma, float * mb, float * mc, const size_t nx, const size_t ny) {
  size_t ix = threadIdx.x + blockIdx.x * blockDim.x;
  size_t id = 0;
  for (size_t iy=0; iy<ny; iy++) {
    id = iy * nx + ix; 
    if (ix < nx) mc[id] = ma[id] + mb[id];
  }
}
...
int main(int argc, char ** argv) {
  ...
  // const unsigned int tpb_y = 1;
  dim3 threads(tpb_x, 1);
  dim3 blocks((nx+threads.x-1)/threads.x, 1);
